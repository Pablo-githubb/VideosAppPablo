<x-videos-app-layout>
    ...
    <div class="form-container">
        <div class="form-header">
            <h1>Afegir Vídeo</h1>
            <p>Completa el formulari per crear un nou vídeo a la plataforma.</p>
        </div>

        <div class="form-card">
            <form method="POST" action="{{ route('videos.manage.store') }}" data-qa="form-create-video">
                @csrf

                <div class="form-group">
                    <label for="title">Títol</label>
                    <input type="text" id="title" name="title" value="{{ old('title') }}" required data-qa="input-title" placeholder="Introdueix el títol del vídeo">
                    @error('title')
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" required data-qa="input-description" placeholder="Descriu el contingut del vídeo">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="error-msg">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="url" id="url" name="url" value="{{ old('url') }}" required data-qa="input-url" placeholder="https://www.youtube.com/watch?v=...">
                    @error('url')
                        <p class="error-msg">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn-submit" data-qa="btn-submit">Crear Vídeo</button>
                </div>
            </form>
        </div>
    </div>
