<x-videos-app-layout>
    <style>
        ...
        .videos-list { text-align: left; background: rgba(0, 0, 0, 0.2); border-radius: 8px; padding: 1rem; margin-bottom: 2rem; max-height: 150px; overflow-y: auto; border: 1px solid rgba(255, 255, 255, 0.05); }
        .videos-list h3 { font-size: 0.875rem; color: #fff; margin-bottom: 0.5rem; border-bottom: 1px solid rgba(255, 255, 255, 0.1); padding-bottom: 0.25rem; }
        .videos-list li { font-size: 0.8125rem; color: #d4d4d8; list-style: inside disc; margin-bottom: 0.25rem; }
    ...
    <div class="delete-container">
        ...
        <h1 class="delete-title">Confirmar Eliminació</h1>
        <p class="delete-text">
            Estàs completament segur que vols eliminar la sèrie <strong>"{{ $serie->title }}"</strong>? Aquesta acció no es pot desfer.
        </p>

        @if($serie->videos->count() > 0)
            <div class="videos-list">
                <h3>Vídeos afectats (es desassignaran de la sèrie):</h3>
                <ul>
        ...
        <form action="{{ route('series.manage.destroy', $serie->id) }}" method="POST">
            ...
            <div class="delete-actions">
                ...
                <button type="submit" class="btn btn-delete" data-qa="btn-confirm-delete-serie">Eliminar Sèrie</button>
            </div>
        </form>
    </div>
</x-videos-app-layout>
