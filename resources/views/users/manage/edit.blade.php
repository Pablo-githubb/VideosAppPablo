<x-videos-app-layout>
    ...
    <div class="form-container">
        <div class="form-header">
            <h1>Editar Usuari</h1>
            <p>Modifica les dades de l'usuari <strong>{{ $user->name }}</strong>.</p>
        </div>
        ...
        <form method="POST" action="{{ route('users.manage.update', $user->id) }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" class="form-control" data-qa="user-name" value="{{ old('name', $user->name) }}" required placeholder="Ex. Joan Garcia">
                @error('name')
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" id="email" class="form-control" data-qa="user-email" value="{{ old('email', $user->email) }}" required placeholder="Ex. joan@iesebre.com">
                @error('email')
                    <span class="error-msg">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                <label for="password">Contrasenya (deixar en blanc per mantenir-la)</label>
                <input type="password" name="password" id="password" class="form-control" data-qa="user-password" placeholder="Mínim 8 caràcters si es vol canviar">
                @error('password')
                    <span class="error-msg">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group checkbox-group">
                <input type="checkbox" name="super_admin" id="super_admin" data-qa="user-super-admin" value="1" {{ old('super_admin', $user->isSuperAdmin()) ? 'checked' : '' }}>
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn btn-primary" data-qa="btn-submit-user">Actualitzar Usuari</button>
            </div>
        </form>
    </div>
</x-videos-app-layout>
